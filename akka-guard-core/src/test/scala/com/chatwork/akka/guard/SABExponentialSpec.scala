...
class SABExponentialSpec
  ...
  "SABExponential" - {
    "auto reset" in {
      ...
      for {_ <- 1 to 10}
        (sabBroker ? message2).mapTo[String].failed.futureValue
      ...
      awaitCond(
        (messageRef ? SABActor.GetStatus)
          .mapTo[SABStatus].futureValue == SABStatus.Closed, 3 seconds, 1 seconds
      )
      ...
      awaitCond(
        (messageRef ? SABActor.GetStatus)
          .mapTo[SABStatus].futureValue == SABStatus.Closed, 5 seconds, 1 seconds
      )
      ...
      awaitCond(
        (messageRef ? SABActor.GetStatus)
          .mapTo[SABStatus].futureValue == SABStatus.Closed, 6 seconds, 1 seconds
      )
